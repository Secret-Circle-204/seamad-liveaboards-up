      ...
      <div className='w-full flex flex-wrap'>
        <section className='trips_page relative w-full h-full  pt-3  text-gray-600 body-font max-w-[1230px]  mx-auto  '>
          {currentMonth !== filter.start_date && (
            <div
              key={currentMonth}
              className='max-w-[1230px] w-full lg:px-0 px-4 mx-auto my-6 '
            >
          ...
          <div
            key={plan?.id}
            className='max-w-[1230px] lg:px-0 px-4 w-full mx-auto my-3 '
          >
            <table className='  bg-gray-200 max-sm:p-2  lg:bg-transparent  my-table table-default uk-table jrt jrt-instance-1   '>
              ...
              <tbody>
                <tr className=' '>
                  ...
                  {/* // if plaan.available_places === 0 set status to completed */}
                  <td className='jrt-cell-8 sm:text-center sm:items-center lg:max-w-[130px] sm:py-14   text-gray-900'  >
                    <button className={`  ${plan?.available_places === 0 ? 'completed p-2 bg-yellow-300  text-white    hover:bg-yellow-400  hover:text-white  rounded    duration-[2500ms] ease-out transition '
                      : trip_status === 'not_available' ? 'bg-red-500 text-white hover:bg-red-600 px-1 py-2 hover:cursor-not-allowed  rounded     ' : ' bg-green-400 hover:bg-green-500 text-white  animate-bounce duration-[2500ms] ease-out transition   rounded px-4 py-2  hover:animate-none  '} 
                    `} >

                      {plan?.available_places === 0 ? 'completed' : trip_status}
                  ...
                  <td className="uk-button w-20 sm:pr-5 text-center">
                    < button
                      ...
                      onClick={() => {
                        window.location.href = `/booking/${plan?.id}`
                      }}
                      className={`  ${trip_status === 'not_available' || plan?.available_places === 0 ? 'disabled p-2 bg-red-500  text-white    hover:bg-red-600  hover:text-white  rounded    duration-[2500ms] ease-out transition '
                        : ' bg-blue3 hover:bg-dark text-white      rounded px-4 py-2     '} 
                      `}
                    ...
                    <Link
                      className=' flex  mt-[8px] justify-center text-center  text-white bg-blue-900   border-0 py-2 items-center px-3 focus:outline-none hover:bg-dark rounded text-lg'
                      href={'/specialBook'}
                      title='title'
                    // defaultValue={titlep}
                    >
