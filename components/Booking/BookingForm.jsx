...
export default function TripsForm({ trip, modalIsOpen, setModalIsOpen }) {
  ...
  const [total_price, setTotalPrice] = useState(trip?.price);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.save(`${trip?.trip?.trip_name}-${ticketNumber}.pdf`);
    if (pdf) {
      setLoading(true);
      setTimeout(() => {
        setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('trip', trip?.trip?.trip_name);
    ...
    const sendDataForm = {
      ...
      email,
      phone,
      trip: trip?.trip?.trip_name,
      check_in_date : bookingDate,
      ticket_number: ticketNumber,
      country,
      number_of_guests,
      total_price: trip?.price * number_of_guests,
      message,
    };
...
// return (
//   <div className='booking-form p-6 bg-gray-100 dark:bg-gray-800 rounded-md shadow-md mt-4'>
//     <h3 className='text-xl font-bold'>Book Your Trip</h3>
//     {booked ? (
//       <BookingTicket trip={trip} formData={formData} />
//     ) : (
...
//         </div>
//         <div className='mb-4'>
//           <label className='block text-gray-700 dark:text-gray-300'>Number of Guests</label>
//           <input type='number' name='guests' value={formData.guests} onChange={handleChange} className='w-full p-2 border rounded' min='1' />
//         </div>
...
// 'use client'
// // components/AllTrips/BookingForm.jsx
// import { useState } from 'react';

// export default function BookingForm({ plan }) {
...
//     e.preventDefault();
//     try {
//       // API call to book the trip
//       const response = await fetch('/api/book-trip', {
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({ planId: plan.id, ...bookingDetails }),
//       });
...
//   return (
//     <form onSubmit={handleBooking} className="booking-form p-4 bg-white shadow-md rounded">
//       <h3 className="text-xl font-bold mb-4">Book Your Trip</h3>
//       <div className="mb-4">
//         <label htmlFor="name" className="block text-gray-700 text-sm font-bold mb-2">Name:</label>
//         <input type="text" id="name" value={bookingDetails.name} onChange={(e) => setBookingDetails({ ...bookingDetails, name: e.target.value })} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" />
//       </div>
//       <div className="mb-4">
//         <label htmlFor="email" className="block text-gray-700 text-sm font-bold mb-2">Email:</label>
//         <input type="email" id="email" value={bookingDetails.email} onChange={(e) => setBookingDetails({ ...bookingDetails, email: e.target.value })} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" />
//       </div>
//       <div className="mb-4">
//         <label htmlFor="phone" className="block text-gray-700 text-sm font-bold mb-2">Phone:</label>
//         <input type="text" id="phone" value={bookingDetails.phone} onChange={(e) => setBookingDetails({ ...bookingDetails, phone: e.target.value })} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" />
//       </div>
//       <div className="mb-4">
//         <label htmlFor="numberOfGuests" className="block text-gray-700 text-sm font-bold mb-2">Number of Guests:</label>
//         <input type="number" id="numberOfGuests" value={bookingDetails.numberOfGuests} onChange={(e) => setBookingDetails({ ...bookingDetails, numberOfGuests: parseInt(e.target.value) })} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" />
//       </div>
//       <button type="submit" disabled={isBooked} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
